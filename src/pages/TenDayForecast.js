...
const TenDayForecast = () => {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            // Filter the forecast to show one reading per day for 10 days
            const tenDayForecast = data.list.filter((_, index) => index % 8 === 0).slice(0, 10);
            setWeatherData(tenDayForecast);
        } catch (err) {
